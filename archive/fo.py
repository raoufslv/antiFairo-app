...
fire_conf_threshold = 0.5  # Adjust as needed
smoke_conf_threshold = 0.5  # Adjust as needed
...
class FireSmokeDetectorApp(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.cameras = {"laptop": defaultCamera}
        self.current_camera_label = "laptop"
        self.capture = None
        self.initUI()
        ...
        self.fire_detected_signal.connect(self.handle_fire_detection)  # Connect signal to slot
    ...
    def initUI(self):
        ...
        self.camera_list.addItem("laptop")
        ...
        try:
            with open('camera_config.json', 'r') as f:
                ...
                for label, address in self.cameras.items():
                    self.camera_list.addItem(label)
    ...
    def add_camera(self):
        ...
        if ok and camera_label:
            ...
            if ok and camera_address:
                ...
                self.camera_list.addItem(camera_label)
    ...
    def select_camera(self, item):
        self.current_camera_label = item.text()
        self.stop_streaming()
        ...
        if self.capture is not None:
            ...
            if ret:
                ...
                if not self.background_processing and self.processed_frame is not None:
                    qt_image = QtGui.QImage(self.processed_frame.data, self.processed_frame.shape[1], self.processed_frame.shape[0], self.processed_frame.strides[0], QtGui.QImage.Format_RGB888)
        ...
        while self.capture is not None and self.capture.isOpened():
            if self.current_frame is not None:
                ...
                if self.frame_count % self.frame_skip == 0:
                    ...
                    self.processed_frame = self.plot_boxes(results, resized_frame, img)

                    if self.fire_detected or self.smoke_detected:
                        self.save_detection_frame(original_frame, self.processed_frame, results)
    ...
    def save_detection_frame(self, original_frame, processed_frame, results):
        ...
        cv2.imwrite(f'detections/frame_{timestamp}_original.jpg', cv2.cvtColor(original_frame, cv2.COLOR_RGB2BGR))
        cv2.imwrite(f'detections/frame_{timestamp}_processed.jpg', cv2.cvtColor(processed_frame, cv2.COLOR_RGB2BGR))

        # Save YOLO annotation
        with open(f'detections/frame_{timestamp}.txt', 'w') as f:
            for det in results:
                if len(det):
                    for *xyxy, conf, cls in det:
                        if (int(cls) == 0 and conf >= fire_conf_threshold) or (int(cls) == 1 and conf >= smoke_conf_threshold):
                            x_center = (xyxy[0] + xyxy[2]) / 2 / processed_frame.shape[1]
                            y_center = (xyxy[1] + xyxy[3]) / 2 / processed_frame.shape[0]
                            width = (xyxy[2] - xyxy[0]) / processed_frame.shape[1]
                            height = (xyxy[3] - xyxy[1]) / processed_frame.shape[0]
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in det:
                    if int(cls) == 0 and conf >= fire_conf_threshold:  # Fire class
                        ...
                        self.draw_box(frame, xyxy, label, color=(0, 0, 255))  # Red color in BGR
                        self.fire_detected = True
                    elif int(cls) == 1 and conf >= smoke_conf_threshold:  # Smoke class
                        label = f'{model.names[int(cls)]} {conf:.2f}'
                        self.draw_box(frame, xyxy, label, color=(128, 128, 128))  # Gray color in BGR
